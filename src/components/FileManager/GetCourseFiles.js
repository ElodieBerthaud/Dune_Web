...
class GetCourseFiles extends Component {
    ...
    handleChange = (event, value) => {
        if (event.target.name !== 'recherche')
            this.setState({ [event.target.name]: event.target.value }, this.searchFilters);
        else
    ...
    openManage = (id, nom, desc, owner, privateF) => {
        ...
        this.setState({ manage: true, manageId: id, fileTitre:  nom, fileDesc: desc, fileOwner: ownerB, filePrivate: privateF});
    ...
    getFilesList = () => {
        ...
        if (obj !== null){
            ...
            for (var i = 0; i < nbFiles; i++)
            {
                ...
                files.push(
                    <div className={classes.filesList} key={i}>
                        <Card className={classes.card} classes={{root: classes.card}} onClick={this.openManage.bind(this, id,
                            obj[i].nom,
                            obj[i].description,
                            obj[i].idUser,
                            obj[i].private)}>
            ...
            if (nbFiles === 0){
                files.push(<div className="textCenter">
                                ...
                                <FormControlLabel
                                    control={
                                        <Button variant="contained" size="large" color="primary"
                                                className={classes.margin} onClick={this.searchFilters}>
                                            Rechercher
                                        </Button>
                                    }
                                />
...
const mapDispatchToProps = dispatch => {
    ...
    return {
        getFiles: (token, filesPrivate, title, typeF, classement) => dispatch({ type: "GET_FILES_REQUEST", token, filesPrivate, title, typeF, classement })
    };
