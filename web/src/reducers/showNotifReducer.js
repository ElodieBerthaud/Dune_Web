...
export default function showNotif(state = initialState.showNotif, action) {
    switch (action.type) {
        ...
        case SHOW_NOTIF_SUCCESS:
            return { ...state,  success: true, error: false, contentProf: action.contentProf, nomApp: action.nomApp, idApp: action.idApp, typeNotif: action.typeNotif, idNotif: action.idNotif, idDemande: action.idDemande, isAccepted: action.isAccepted};
