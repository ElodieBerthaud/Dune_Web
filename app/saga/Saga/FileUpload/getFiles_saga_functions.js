...
export function* update_files(datas) {
  ...
  try {
    ...
    if (response.data.status === 200) {
      ...
      yield put({
        type: 'SNACK_PUT_SUCCESS', message: 'Le fichier a bien été mit à jour.', redirect: true, pathToRedirect: '/cours'
      });
    } else {
      yield put({ type: 'END_LOADING' });
...
export function* delete_file(datas) {
  try {
    ...
    if (response.data.status === 200) {
      yield put({
        type: SNACK_PUT_SUCCESS, message: 'Votre fichier a bien été supprimé.', redirect: true, pathToRedirect: '/cours'
      });
