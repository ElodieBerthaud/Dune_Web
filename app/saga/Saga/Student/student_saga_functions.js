...
import {
  get_all_students_api, get_nbr_students_api, student_profile_api, add_student_api, get_student_results_api, update_student_api
} from '../../Api/Students/students_api_functions';
...
export function* get_all_students(datas) {
  try {
    ...
    if (response.data.status === 200) {
      yield put({ type: GET_STUDENTS_SUCCESS, content });
    } else {
...
export function* student_profile(datas) {
  ...
  try {
    ...
    if (response.data.status === 200) {
      ...
      yield put({
        type: 'STUDENT_PROFILE_SUCCESS', nomEleve, prenomEleve, noEleve, idEleve, picEleve
      });
...
export function* addStudent(datas) {
  ...
  try {
    ...
    if (response.data.status === 200) {
      ...
      yield put({
        type: 'SNACK_PUT_SUCCESS', message: 'Le nouvel eleve a bien ete ajoute.', redirect: true, pathToRedirect: '/students'
      });
    }
