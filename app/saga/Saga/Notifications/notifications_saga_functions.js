...
import {
  get_nb_notifs_api, get_all_notifs_api, get_a_notif_api, validate_app_api, read_notif_api
} from '../../Api/Notifications/notifications_api_functions';
...
export function* getNotifs(datas) {
  try {
    ...
    if (response.data.status === 200 && response2.data.status === 200) {
      ...
      yield put({ type: 'GET_NOTIFS_SUCCESS', nbNotif, content });
    }
...
export function* showNotif(datas) {
  ...
  try {
    ...
    if (responseNotif.data.status === 200) {
      ...
      const datasProf = { idDemande, token: datas.token };
      ...
      if (responseProf.data.status === 200) {
        ...
        if (responseApp.data.status === 200) {
          ...
          yield put({
            type: 'SHOW_NOTIF_SUCCESS', contentProf: responseProf.data.response, nomApp, idApp, typeNotif, idNotif, idDemande, isAccepted
          });
...
export function* validateApp(datas) {
  try{
    ...
    if (response.data.status === 200) {
      ...
      const response2 = yield call(read_notif_api, datas);

      if (response2.data.status === 200) {

        yield put({
          type: "SNACK_PUT_SUCCESS",
          message: "Votre reponse a bien ete prise en compte.",
          redirect: true,
