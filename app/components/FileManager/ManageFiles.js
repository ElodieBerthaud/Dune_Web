...
class ManageFiles extends Component {
  ...
  componentWillUpdate(nextProps, nextState, nextContext) {
    if (this.state.manage !== nextProps.manage) this.setState({ manage: nextProps.manage });
    if (this.state.owner !== nextProps.owner) this.setState({ owner: nextProps.owner });
    if (this.state.private !== nextProps.private) this.setState({ private: nextProps.private });
    if (this.state.idFile !== nextProps.id) this.setState({ idFile: nextProps.id });
  }
    ...
    handleUpdate = () => {
      ...
      if (this.state.desc.replace(/ /g, '') === '' && this.state.name.replace(/ /g, '') === '') {
        this.props.error();
      } else if (this.state.desc.replace(/ /g, '') === '') {
        this.props.updateFile(this.props.token, this.state.name, this.props.desc, this.state.private, this.state.idFile);
      } else if (this.state.name.replace(/ /g, '') === '') {
        this.props.updateFile(this.props.token, this.props.titre, this.state.desc, this.state.private, this.state.idFile);
      } else {
...
const mapDispatchToProps = (dispatch) => ({
  updateFile: (token, newTitle, newDesc, privateF, idFile) => dispatch({
    type: 'UPDATE_FILE', token, newTitle, newDesc, privateF, idFile
  }),
  deleteFile: (token, idFile) => dispatch({ type: 'DELETE_FILE', token, idFile }),
  error: () => dispatch({ type: 'SNACK_PUT_ERROR', message: 'Vous ne pouvez pas modifier avec des valeurs vides.' })
});
