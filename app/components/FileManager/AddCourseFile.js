...
import PdfViewer from '../Viewer/test';
...
class AddCourseFile extends Component {
    ...
    handleChange = (event) => {
      if (event.target.name === 'file' && event.target.value !== null) {
        ...
        this.setState({
          openPDF: true, urlPDF: file_obj, fileName: file_obj.name, file: file_obj
        });
      } else {
    ...
    handleNext = () => {
      if (this.state.activeStep === 0) {
        if (this.state.cath === 0) {
          ...
          const elem = document.getElementsByName('cath');
    ...
    getStepContent = (step, classes) => {
      switch (step) {
        case 0:
          return (
            <div> <p> Choisissez la categorie de media que vous allez ajouter.</p>
        ...
        case 1:
          return (
            <div> <p> Remplissez les champs ci-dessous (20 caractères maximum)</p>
        ...
        case 2:
          return (
            <div>
              <p>
                        Choisissez le fichier que vous voulez ajouter a votre espace. Vous pouvez choisir de le rendre privé en cochant la case "fichier privé". Sinon, le fichier sera automatiquement partagé pour tout l'école.
              </p>
              <FormGroup row style={{ margin: '5%' }}>
                ...
                <FormControlLabel
                  control={(
                    <Tooltip title="Garder ce fichier pour moi" placement="top">
                      <Checkbox
        ...
        case 3:
          return (
            <div> <p> Veuillez remplir une description pour ce document. </p>
      ...
      return (
        <div>
          ...
          {this.state.openPDF === true && this.state.cath === 'PDF'
            ...
            ? <PdfViewer mode={'apercu'} open={this.state.openPDF} url={this.state.urlPDF} />
...
const mapDispatchToProps = (dispatch) => ({
  ...
  AddFile: (token, fileName, description, prive, fileUser, fileType) => dispatch({
    type: 'UPLOAD_FILE_REQUEST', token, fileName, description, prive, fileUser, fileType
  })
});
